...
import { DailyResponseSchema, RunRateFilterSchema } from "@/lib/validators/run-rate";
import { getDailyPerformance } from "@/server/run-rate/run-rate-repository";
...
export const GET = async (req: NextRequest) => {
  try {
    ...
    const response = DailyResponseSchema.parse(await getDailyPerformance(parsed.data));
    return NextResponse.json(apiSuccess(response));
  } catch (err) {
    return routeError("GET /api/run-rate/daily", err);
  }
};
